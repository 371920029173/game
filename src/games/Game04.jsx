    ...
    <div style={{ width: 420, margin: '0 auto' }}>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: `repeat(${SIZE}, 1fr)`, gap: 2, background: '#bdbdbd', padding: 4, borderRadius: 8 }}>
        {board.map((row, y) => row.map((cell, x) => (
          <div key={x + '-' + y} onClick={() => handleClick(x, y)} style={{ width: 24, height: 24, background: '#fff', borderRadius: '50%', boxShadow: '0 1px 2px #0001', display: 'flex', alignItems: 'center', justifyContent: 'center', cursor: over || cell ? 'not-allowed' : 'pointer', border: '1px solid #e5e7eb', fontSize: 18, color: cell === 1 ? '#111' : '#fbbf24' }}>
            {cell === 1 ? '●' : cell === 2 ? '○' : ''}
          </div>
        )))}
      </div>
